...
sub sec_throttle {
    if (vsthrottle.is_denied(req.http.X-Actual-IP, 500, 1s) ||
        vsthrottle.is_denied(req.http.X-Actual-IP, 2000, 30s) ||
        vsthrottle.is_denied(req.http.X-Actual-IP, 10000, 5m)) {
