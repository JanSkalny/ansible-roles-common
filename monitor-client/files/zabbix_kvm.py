...
def main():
  ...
  if options.resource == "pool":
    ...
    elif options.action == "active-count":
      r = domain_active_count(options)
    elif options.action == "active":
...
def domain_active_count(options):
  conn = kvm_connect()
  cnt = 0
  try:
    conn.listAllDomains(0)
    ...
    domains = []
    for dom_id in conn.listDomainsID():
      if conn.lookupByID(dom_id).isActive():
        cnt = cnt + 1
  else:
    for domain in conn.listAllDomains(0):
      if domain.isActive():
...
def parse_args():
  ...
  if options.resource == "pool":
    ...
    domain_valid_actions = [ 'list', 'active', 'active-count', 'UUID' ]
